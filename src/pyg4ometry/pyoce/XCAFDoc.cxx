...
PYBIND11_MODULE(XCAFDoc, m) {
  ...
  py::class_<XCAFDoc_DocumentTool, opencascade::handle<XCAFDoc_DocumentTool>,
             TDF_Attribute>(m, "XCAFDoc_DocumentTool")
      .def_static("ShapeTool", &XCAFDoc_DocumentTool::ShapeTool)
      .def_static("ColorTool", &XCAFDoc_DocumentTool::ColorTool);

  py::class_<XCAFDoc_ShapeMapTool, opencascade::handle<XCAFDoc_ShapeMapTool>,
             TDF_Attribute>(m, "XCAFDoc_ShapeMapTool")
      .def(py::init<>())
      .def_static("GetID", &XCAFDoc_ShapeMapTool::GetID);

  py::class_<XCAFDoc_Location, opencascade::handle<XCAFDoc_Location>,
             TDF_Attribute>(m, "XCAFDoc_Location")
      .def(py::init<>())
      .def("Get", &XCAFDoc_Location::Get)
      .def_static("GetID", &XCAFDoc_Location::GetID);

  py::class_<XCAFDoc_ShapeTool, opencascade::handle<XCAFDoc_ShapeTool>>(
      m, "XCAFDoc_ShapeTool")
      .def(py::init<>())
      .def("BaseLabel", &XCAFDoc_ShapeTool::BaseLabel)
      .def("Dump",
           [](XCAFDoc_ShapeTool &st) {
      ...
      .def("GetReferredShape",
           [](XCAFDoc_ShapeTool &st, const TDF_Label &label1,
              TDF_Label &label2) { st.GetReferredShape(label1, label2); })
      .def("GetShape",
           [](XCAFDoc_ShapeTool &st, const TDF_Label &label,
              TopoDS_Shape &shape) { return st.GetShape(label, shape); })
      .def("GetShape",
           [](XCAFDoc_ShapeTool &st, const TDF_Label &label) {
      ...
      .def("IsAssembly",
           [](XCAFDoc_ShapeTool &st, const TDF_Label &label) {
             return st.IsAssembly(label);
           })
      .def("IsComponent",
           [](XCAFDoc_ShapeTool &st, const TDF_Label &label) {
             return st.IsComponent(label);
           })
      .def("IsCompound",
           [](XCAFDoc_ShapeTool &st, const TDF_Label &label) {
             return st.IsCompound(label);
           })
      .def("IsFree", [](XCAFDoc_ShapeTool &st,
                        const TDF_Label &label) { return st.IsFree(label); })
      .def("IsReference",
           [](XCAFDoc_ShapeTool &st, const TDF_Label &label) {
             return st.IsReference(label);
           })
      .def("IsShape", [](XCAFDoc_ShapeTool &st,
                         const TDF_Label &label) { return st.IsShape(label); })
      .def("IsSimpleShape",
           [](XCAFDoc_ShapeTool &st, const TDF_Label &label) {
             return st.IsSimpleShape(label);
           })
      .def("IsSubShape",
           [](XCAFDoc_ShapeTool &st, const TDF_Label &label) {
             return st.IsSubShape(label);
           })
      .def("IsTopLevel",
           [](XCAFDoc_ShapeTool &st, const TDF_Label &label) {
             return st.IsTopLevel(label);
           })
